...
class PR2d(nn.Module):
    ...
    def output_PR(self, lambda1, lambda2, alpha, weights_pole1, weights_pole2, weights_residue):
        ...
        output_residue1=torch.einsum("biox,xoikpq->bkox", alpha, Hw) 
        output_residue2=torch.einsum("biox,xoikpq->bkpq", alpha, Pk) 
    ...
    def forward(self, x):
        ...
        term3=torch.einsum("bipz,biqx->bipqxz", torch.exp(term1),torch.exp(term2))
...
for ep in range(epochs):
    ...
    for x, y in train_loader:
        ...
        optimizer.zero_grad()
        out = model(x).reshape(batch_size_train, s1, s2)
    ...
    with torch.no_grad():
        ...
        for x, y in test_loader:
            x, y = x.cuda(), y.cuda()
            out = model(x).reshape(batch_size_vali, s1, s2)
