...
class operator_model:
    ...
    def NOMAD(self, params, inputs):
        ...
        b = self.branch_apply(branch_params, inputsu.reshape(inputsu.shape[0], inputsu.shape[2], inputsu.shape[1])) 
        b = jnp.tile(b, (1,inputsy.shape[1]//b.shape[1],1))
...
def main(n, decoder):
    ...
    data = sio.loadmat(TRAIN_PATH)
    U_train   = data['u'][:num_train, :, :, :T_in]  # N x Nx
    s_train   = data['u'][:num_train, :,  :, T_in:T_in+T] # N x Nt x Nx

    x = np.linspace(0,1,num=Nx)
    y = np.linspace(0,1,num=Ny)
    t = np.linspace(0,50,num=Nt)
    t = t[T_in:T_in+T]

    data = sio.loadmat(TEST_PATH )
    U_test    = data['u'][-num_test:,:,:,:T_in]
    s_test    = data['u'][-num_test:,:,:,T_in:T_in+T]

    XX, YY, TT = np.meshgrid(x,y,t)

    y_train = jnp.tile(np.concatenate((XX.flatten()[:,None],YY.flatten()[:,None],TT.flatten()[:,None]),axis=-1)[None,...],(num_train,1,1))
    y_test  = jnp.tile(np.concatenate((XX.flatten()[:,None],YY.flatten()[:,None],TT.flatten()[:,None]),axis=-1)[None,...],(num_test,1,1))
    ...
    U_train = jnp.reshape(U_train,(num_train,m,du)) # (1000, 256, 1)
    y_train = jnp.reshape(y_train,(num_train,P,dy)) # (1000, 25600, 2)
    s_train = jnp.reshape(s_train,(num_train,P,ds)) # (1000, 25600, 1)
    ...
    if decoder=="nonlinear":
        branch_layers = [m,   256,256,256,256, ds*n]
        trunk_layers  = [ds*n*2, 256,256,256,256, ds]
    elif decoder=="linear":
    ...
    # flat_params = np.load('/home/yy/yy/NO/NOMAD-main/model/NS_iter50000_n12_decodernonlinear_250424_11_19_04_params.npy')
    ...
    np.save(f'model/NS_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_params.npy', flat_params)
    np.save(f'model/NS_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_losses.npy', np.array([model.loss_log]))

    s_pred_test = np.zeros_like(s_test)
    for i in range(0,num_test,10):
        idx = i + np.arange(0,10)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n', metavar='n', type=int, nargs='+', default=12,help='Latent dimension of the solution manifold')
    parser.add_argument('--decoder', metavar='decoder', type=str, nargs='+',default='nonlinear', help='Type of decoder. Choices a)"linear" b)"nonlinear"')
