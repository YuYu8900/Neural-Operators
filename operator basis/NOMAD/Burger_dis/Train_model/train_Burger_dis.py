...
def main(n, decoder):
    ...
    data = sio.loadmat('/home/yy/yy/data/burgers_data_512_51.mat')
    x, t, u_train = data["x"], data["t"], data["sol"]  # N x nt x nx
    U_train   = u_train[:num_train, :, :T_in]  # N x Nx
    s_train   = u_train[:num_train, :, T_in:T_in+T] # N x Nt x Nx

    x = np.linspace(0,1,num=Nx)
    t = np.linspace(0,1,num=Nt)
    t = t[T_in:T_in+T]

    U_test    = u_train[-num_test:, :, :T_in]
    s_test    = u_train[-num_test:, :, T_in:T_in+T]

    TT, XX = np.meshgrid(t,x,indexing='ij') # 100 x 256

    y_train = jnp.tile(np.concatenate((TT.flatten()[:,None],XX.flatten()[:,None]),axis=-1)[None,...],(num_train,1,1)) # (1000, 25600, 2)
    ...
    np.save(f'/home/yy/yy/NO/NOMAD-main/model/Burger_dis_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_params.npy', flat_params)
    np.save(f'/home/yy/yy/NO/NOMAD-main/model/Burger_dis_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_losses.npy', np.array([model.loss_log]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n', metavar='n', type=int, nargs='+', default=10,help='Latent dimension of the solution manifold')
