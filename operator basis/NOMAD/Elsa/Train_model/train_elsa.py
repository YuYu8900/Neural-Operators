...
def main(n, decoder):
    ...
    input_s = input_s.transpose(1,0)
    # input_s = jnp.tile(input_s,(1,1,1))
    input_xy = np.load(PATH_XY).transpose(2,0,1)

    U_train = input_s[:num_train]
    s_train = input_xy[:num_train]
    U_test = input_s[-num_test:]
    s_test = input_xy[-num_test:]

    x = np.linspace(0,1,num=Nx)
    y = np.linspace(0,1,num=Nx)

    y_train = jnp.tile(np.concatenate((x.flatten()[:,None],y.flatten()[:,None]),axis=-1)[None,...],(num_train,1,1)) # (1000, 25600, 2)
    y_test  = jnp.tile(np.concatenate((x.flatten()[:,None],y.flatten()[:,None]),axis=-1)[None,...],(num_test,1,1))

    U_train = jnp.asarray(U_train) # (1000, 500)
    s_train = jnp.asarray(s_train) # (1000, 500)
    ...
    if decoder=="nonlinear":
        ...
        branch_layers = [m,   128, 128, 128, 128, ds*n]
        trunk_layers  = [dy,  128, 128, 128, 128, ds*n]
    ...
    # np.save(f'model/Elsa_hid128_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_params.npy', flat_params)
    # np.save(f'model/Elsa_hid128_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_losses.npy', np.array([model.loss_log]))
    
    # flat_params = np.load('model/Burger_iter200000_n10_decodernonlinear_250423_21_15_11_params.npy')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n', metavar='n', type=int, nargs='+', default=20,help='Latent dimension of the solution manifold')
    parser.add_argument('--decoder', metavar='decoder', type=str, default='nonlinear', nargs='+', help='Type of decoder. Choices a)"linear" b)"nonlinear"')
