    ...
    def train(self, train_dataset, test_dataset, nIter = 10000):
        ...
        for it in pbar:
            ...
            if it % 100 == 0:
                ...
                pbar.set_postfix({'Train error':   errorTrain, 
...
def main(n, decoder):
    PATH_Train = /Darcy_421/piececonst_r421_N1024_smooth1.mat'
    PATH_Test = /Darcy_421/piececonst_r421_N1024_smooth2.mat'
    ...
    data = sio.loadmat(PATH_Train) # N x nt x nx
    U_train   = data['coeff'][:num_train,::sub,::sub][:,:h,:h]
    s_train   = data['sol'][:num_train,::sub,::sub][:,:h,:h]

    data = sio.loadmat(PATH_Test)
    U_test    = data['coeff'][:num_test,::sub,::sub][:,:h,:h]
    s_test    = data['sol'][:num_test,::sub,::sub][:,:h,:h]

    x = np.linspace(0,1,num=Nx)
    y = np.linspace(0,1,num=Ny)

    XX,YY = np.meshgrid(x,y,indexing='ij') # 100 x 256

    y_train = jnp.tile(np.concatenate((XX.flatten()[:,None],YY.flatten()[:,None]),axis=-1)[None,...],(num_train,1,1)) # (1000, 25600, 2)
    y_test  = jnp.tile(np.concatenate((XX.flatten()[:,None],YY.flatten()[:,None]),axis=-1)[None,...],(num_test,1,1))
    ...
    np.save(f'model/Darcy_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_params.npy', flat_params)
    np.save(f'model/Darcy_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_losses.npy', np.array([model.loss_log]))
