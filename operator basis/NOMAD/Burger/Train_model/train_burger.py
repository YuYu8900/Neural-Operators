...
def main(n, decoder):
    ...
    data = sio.loadmat('data/burgers_data_R10.mat')
    x_data = data['a'][:,::sub]
    y_data = data['u'][:,::sub]

    U_train = x_data[:num_train,:]
    s_train = y_data[:num_train,:]
    U_test = x_data[-num_test:,:]
    s_test = y_data[-num_test:,:]

    x = np.linspace(0,1,num=Nx)

    y_train = jnp.tile(x.flatten()[:,None][None,...],(num_train,1,1)) # (1000, 25600, 2)
    y_test  = jnp.tile(x.flatten()[:,None][None,...],(num_test,1,1))
    ...
    if decoder=="nonlinear":
        branch_layers = [m,   128, 128, ds*n]
        trunk_layers  = [ds*n*2, 128, 128, ds]
    elif decoder=="linear":
    ...
    np.save(f' /model/Burger_hid128_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_params.npy', flat_params)
    np.save(f' /model/Burger_hid128_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_losses.npy', np.array([model.loss_log]))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n', metavar='n', type=int, nargs='+', default=128,help='Latent dimension of the solution manifold')
