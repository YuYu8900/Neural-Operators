...
def main(n, decoder):
    ...
    U_train   = u_train[:, 0:1, :].reshape(num_train,Nx) # N x Nx
    s_train   = u_train # N x Nt x Nx

    data = np.load('/time_advection/test_IC2.npz')
    x, t, u_test = data["x"], data["t"], data["u"]  # N x nt x nx
    U_test    = u_test[:num_test, 0:1, :]
    s_test    = u_test[:num_test]
    ...
    if decoder=="nonlinear":
        branch_layers = [m,  512,512, ds*n]
        trunk_layers  = [ds*n*2, 512,512, ds]
    elif decoder=="linear":
    ...
    # np.save(f'/model/Advection_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_params.npy', flat_params)
    # np.save(f'/model/Advection_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_losses.npy', np.array([model.loss_log]))
