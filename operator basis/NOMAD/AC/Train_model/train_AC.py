...
def main(n, decoder):
    ...
    data = sio.loadmat(PATH) # N x nt x nx
    U_train   = data['coeff'][:num_train,::r,::r][:,:s,:s]
    s_train   = data['sol'][:num_train,::r,::r][:,:s,:s]

    U_test    = data['coeff'][-num_test:,::r,::r][:,:s,:s]
    s_test    = data['sol'][-num_test:,::r,::r][:,:s,:s]
    ...
    if decoder=="nonlinear":
        branch_layers = [m,  512, 512,512, 512,512, ds*n]
        trunk_layers  = [ds*n*2, 512, 512,512,512,512, ds]
    elif decoder=="linear":
    ...
    # np.save(f'/model/AC_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_params.npy', flat_params)
    # np.save(f'/model/AC_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_losses.npy', np.array([model.loss_log]))
