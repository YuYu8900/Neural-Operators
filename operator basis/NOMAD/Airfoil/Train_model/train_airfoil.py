...
def main(n, decoder):
    ...
    U_train   = input[:num_train, ::r1, ::r2][:, :Nx, :Ny]
    s_train   = output[:num_train, ::r1, ::r2][:, :Nx, :Ny]

    U_test    = input[num_train:num_train+num_test, ::r1, ::r2][:, :Nx, :Ny]
    s_test    = output[num_train:num_train+num_test, ::r1, ::r2][:, :Nx, :Ny]
    ...
    np.save(f'/model/Airfoil_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_params.npy', flat_params)
    np.save(f'/model/Airfoil_iter{TRAINING_ITERATIONS}_n{n}_decoder{decoder}_{t0}_losses.npy', np.array([model.loss_log]))
