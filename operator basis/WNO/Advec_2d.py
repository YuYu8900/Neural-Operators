...
model = WNO2d(width=width, level=level, layers=layers, size=[nx,nt], wavelet=wavelet,
              in_channel=in_channel, grid_range=grid_range, omega=4).to(device)
...
for ep in range(epochs):
    ...
    for x, y in train_loader:
        ...
        optimizer.zero_grad()
        out = model(x).reshape(batch_size, nx, nt)
