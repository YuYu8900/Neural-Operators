...
class WaveConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, level, size, wavelet, mode='symmetric', omega=8):
        ...
        # Parameter initilization
        self.scale = (1 / (in_channels * out_channels))
        self.weights_a1 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.effective_modes_x, self.effective_modes_y, dtype=torch.cfloat))
        self.weights_a2 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.effective_modes_x, self.effective_modes_y, dtype=torch.cfloat))
        self.weights_h1 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.effective_modes_x, self.effective_modes_y, dtype=torch.cfloat))
        self.weights_h2 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.effective_modes_x, self.effective_modes_y, dtype=torch.cfloat))
        self.weights_v1 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.effective_modes_x, self.effective_modes_y, dtype=torch.cfloat))
        self.weights_v2 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.effective_modes_x, self.effective_modes_y, dtype=torch.cfloat))
        self.weights_d1 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.effective_modes_x, self.effective_modes_y, dtype=torch.cfloat))
        self.weights_d2 = nn.Parameter(self.scale * torch.rand(in_channels, out_channels, self.effective_modes_x, self.effective_modes_y, dtype=torch.cfloat))
...
class WaveConv3d(nn.Module):
    def __init__(self, in_channels, out_channels, level, size, wavelet='db4', mode='zero', omega=6):
