...
class CvTencoderdecoder1(nn.Module):
    def __init__(self, image_size, in_channels1, in_channels2,out_dim, dim = 64, kernels=[6, 3, 2], strides=[2, 2, 2],
        ...
        self.stage5_conv_embed = nn.ConvTranspose1d(64, out_dim, kernels[0], strides[0], padding=2)
    ...
    def forward(self, img1, img2):
        ...
        xsr = rearrange(xs3,'b w c -> b c w', w = (self.image_size))
        # xsr = self.stage4_conv_embed(xs3)
        # xsr = self.stage5_conv_embed(xs3)
...
class CvTencoderdecoder2(nn.Module):
    def __init__(self, image_size, in_channels1, in_channels2,out_dim, dim = 64, kernels=[6, 4, 2], strides=[2, 2, 2],
                 heads=[1, 3, 6] , depth = [1, 2, 10], pool='cls', dropout=0., emb_dropout=0., scale_dim=4):
        super().__init__()
