...
class ConvAttention1(nn.Module):
    ...
    def forward(self, x):
        ...
        x = rearrange(x, 'b  w n -> b n w', w=self.img_size)

        q = self.to_q(x)
        q = rearrange(q, 'b (h d) w -> b h w d', h=h)

        v = self.to_v(x)
        v = rearrange(v, 'b (h d) w -> b h w d', h=h)

        k = self.to_k(x)
        k = rearrange(k, 'b (h d) w -> b h w d', h=h)
...
class ConvAttention2(nn.Module):
    ...
    def forward(self, x,y):
        ...
        x = rearrange(x, 'b  w n -> b n w', w= self.img_size)
        y = rearrange(y, 'b  w n -> b n w', w= self.img_size)

        q = self.to_q(y)
        q = rearrange(q, 'b (h d) w -> b h w d', h=h1)
