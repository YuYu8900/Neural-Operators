...
if args.ood == 1:
    ...
    y_ood_og = file['y_ood'].reshape(x_ood_red.shape[0], nx)
...
if not os.path.exists('burger_dis/results/d_' + str(args.latent_dim) + '/' + str(args.method) + '/errors_DON/'):
    os.makedirs('burger_dis/results/d_' + str(args.latent_dim) + '/' + str(args.method) + '/errors_DON/')     
if not os.path.exists('burger_dis/results/d_' + str(args.latent_dim) + '/' + str(args.method) + '/class_DON/'):
    os.makedirs('burger_dis/results/d_' + str(args.latent_dim) + '/' + str(args.method) + '/class_DON/') 
if args.ood == 1:    
    if not os.path.exists('burger_dis/results/d_' + str(args.latent_dim) + '/' + str(args.method) + '/errors_OOD/'):
        os.makedirs('burger_dis/results/d_' + str(args.latent_dim) + '/' + str(args.method) + '/errors_OOD/')   
if args.noise == 1:
    if not os.path.exists('burger_dis/results/d_' + str(args.latent_dim) + '/' + str(args.method) + '/errors_NOISY/'):
        os.makedirs('burger_dis/results/d_' + str(args.latent_dim) + '/' + str(args.method) + '/errors_NOISY/')       
    
# Load autoencoder class
class_AE_dir = 'burger_dis/results/d_' + str(args.latent_dim) + '/' + str(args.method) + '/class_AE/'
...
def preprocess(x, y):
    ...
    n = x.shape[0]
    X_func = x.reshape(n, T_in,ld_sqrt, ld_sqrt, 1) # (#, sqrt(ld), sqrt(ld), 1)
    #print(X_func.shape)

    X_loc = np.linspace(0, 1,T).reshape(T, 1)#
    #print(X_loc.shape)

    y = y.reshape(-1,T,args.latent_dim)
    #print(y.shape)
...
def error_metric(true, pred):
    ...
    pred = np.reshape(pred, (-1, nx))
    true = np.reshape(true, (-1, nx))
...
def show_error(don_model, ae_model, X_func, X_loc, pf_true, n_samples, save, class_dir):
    ...
    if args.method == 'WAE':
        ...
        pf_pred = autoencoder.forward(y_pred).detach().numpy().reshape(n_samples, T,nx)
    else:
        pf_pred = autoencoder.decoder(y_pred).numpy()
        pf_pred = pf_pred.reshape(n_samples,T, nx)    
...
def main():
    ...
    pred = tf.reshape(y_hat, [num_test, T*args.latent_dim])
    ref = tf.reshape(y_test, [num_test,T*args.latent_dim])
    ...
    if True:
        ...
        pf_true = np.reshape(pf_true, (-1,T,nx))
        ...
        if args.ood == 1:
            ...
            pf_true = y_ood_og        
            pf_true = (pf_true - np.min(pf_true))/(np.max(pf_true) - np.min(pf_true))
            pf_true = np.reshape(pf_true, (-1,nx))
