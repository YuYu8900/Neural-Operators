...
#### Load data
file = scipy.io.loadmat("burgers_data_512_51.mat")
s = 512
...
nx,nt = s,50
u = file["sol"].astype(np.float32).transpose(0,2,1)

x_data = u[:n,:T_in,::sub_x].reshape(n*T_in,nx)
y_data = u[:n,T_in:T_in+T,::sub_x]

y_data_re = y_data.reshape(n*T,nx)
...
# x_train = x_train.reshape(n_train,s,1)
# x_test = x_test.reshape(n_test,s,1)

# x_y_data = np.concatenate((x_data, y_data), axis=0) # concatenate inputs and outputs
...
if args.method == 'CAE':
    ...
    x_y_train = y_train.reshape(n_train,nx) # (#, 28, 28) important!
    x_y_test = y_test.reshape(n_test,nx)
...
if args.method == 'vanilla-AE' or args.method == 'MLAE' or args.method == 'CAE':
    ...
    history = autoencoder.fit(y_train, y_train, shuffle=True,
    ...
    decoded_data = autoencoder.predict(y_test).reshape(n_test, nx)
    reference_data = y_test.reshape(n_test, nx)
    ...
    x_y_red = autoencoder.encoder(y_data_norm.reshape(n*T, nx)).numpy()  # encoder

    # Train-test
    # Split x and y data
    y_red = x_y_red.reshape(n,T,args.latent_dim)
    x_red = x_red.reshape(n,T_in,args.latent_dim)
    # Split to train/test
    x_train_red, x_test_red = x_red[:num_train], x_red[-num_test:]
    y_train_red, y_test_red = y_red[:num_train].reshape(num_train,T, args.latent_dim), y_red[-num_test:].reshape(num_test, T,args.latent_dim)
