...
def show_error(don_model, ae_model, X_func, X_loc, pf_true, n_samples, save, class_dir):
    ...
    if args.method == 'WAE':
        ...
        pf_pred = autoencoder.forward(y_pred).detach().numpy().reshape(n_samples, nt,nx)
    else:
        pf_pred = autoencoder.decoder(y_pred).numpy()
        pf_pred = pf_pred.reshape(n_samples,nt, nx)    
...
def main():
    ...
    for i in range(n_epochs+1):
        ...
        if i%50 == 0:
            print('epoch:%d,mse:%f'%(i,val_loss))
        if i == n_epochs:
    ...
    if True:
        ...
        pf_true = np.reshape(pf_true, (-1,nt,nx))
