...
u_train = data["u"].astype(np.float32) 
data1 = np.load("time_advection/test_IC2.npz")
u_test = data1["u"].astype(np.float32) 
u = np.concatenate((u_train, u_test), axis=0)

x_data = u[:n, 0, :].reshape(n,nx) # N x nx
y_data = u[:n, :, :]

y_data_re = y_data.reshape(n*nt,nx)
x_y_data = np.concatenate((x_data, y_data_re), axis=0) # concatenate inputs and outputs
n_samples = x_data.shape[0]

if args.noise == 1:
    # Make noisy inputs (add 10% uncorrelated noise to inputs)
    num_noisy, noise = 50, 0.1
    inputs_n, outputs_n =  u[n:n+num_noisy,0,:], np.array((u))[n:n+num_noisy,:]
    inputs_n = inputs_n.reshape(num_noisy, nx)
    inputs_mean = np.mean(inputs_n)
    outputs_n_re = outputs_n.reshape(num_noisy*nt, nx)

    x_noisy = inputs_n + np.random.normal(0, noise*inputs_mean, (num_noisy, nx))
    x_noisy = x_noisy.reshape(num_noisy, nx)
...
if args.method == 'CAE':
    ...
    x_y_train = x_y_train.reshape(n_train,nx) # (#, 28, 28) important!
    x_y_test = x_y_test.reshape(n_test,nx)
...
if args.method == 'vanilla-AE' or args.method == 'MLAE' or args.method == 'CAE':
    ...
    decoded_data = autoencoder.predict(x_y_test).reshape(n_test, nx)
    reference_data = x_y_test.reshape(n_test, nx)
    ...
    x_y_red = autoencoder.encoder(x_y_data_norm.reshape(n*nt+n, nx)).numpy()  # encoder

    if args.noise == 1:
        # Pass noisy input and output data separately through the encoder
        x_noisy_red = autoencoder.encoder(x_noisy_data_norm.reshape(num_noisy, nx, 1)).numpy()
        y_noisy_red = autoencoder.encoder(y_noisy_data_norm.reshape(num_noisy*nt, nx, 1)).numpy()
    # Train-test
    # Split x and y data
    y_red = x_y_red[n:].reshape(n,nt*args.latent_dim)
    x_red = x_y_red[:n]
    # Split to train/test
    x_train_red, x_test_red = x_red[:num_train], x_red[-num_test:]
