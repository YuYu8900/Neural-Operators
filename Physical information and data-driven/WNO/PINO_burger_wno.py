...
x_train = x_data[:ntrain,:].reshape(ntrain, 1, nx,1).repeat([1, s, 1,1])
y_train = y_data[:ntrain,:,:]
x_test = x_data[-ntest:,:].reshape(ntest, 1, nx,1).repeat([1, s, 1,1])
y_test = y_data[-ntest:,:,:]
x_test_noisy = x_data_noisy[-ntest:,:].reshape(ntest, 1, nx,1).repeat([1, s, 1,1])
# y_test_noisy = y_data_noisy[-ntest:,:,:]
...
index = 0
test_e = torch.zeros(y_test.shape[0])
test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(x_test_noisy, y_test), batch_size=1, shuffle=False)
with torch.no_grad():
    for x, y in test_loader:
        test_l2 = 0
        x, y = x.to(device), y.to(device)
