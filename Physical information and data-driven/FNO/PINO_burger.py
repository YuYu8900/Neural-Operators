...
dataloader = MatReader('burgers_pino.mat',to_torch=False)
x_data =  dataloader.read_field('input')[:, ::sub]
y_data =  dataloader.read_field('output')[:, ::sub_t, ::sub]
x_data_noisy = torch.from_numpy(add_gaussian_noise(x_data)).float()
y_data_noisy = torch.from_numpy(add_gaussian_noise(y_data)).float()
x_data =  torch.from_numpy(x_data).float()
y_data =  torch.from_numpy(y_data).float()
v = dataloader.read_field('visc').item()

x_train = x_data_noisy[:ntrain,:].reshape(ntrain, 1, nx,1).repeat([1, s, 1,1])
y_train = y_data[:ntrain,:,:]
x_test = x_data_noisy[-ntest:,:].reshape(ntest, 1, nx,1).repeat([1, s, 1,1])
y_test = y_data[-ntest:,:,:]
...
# print('64*50')
# index = 0
# test_e = torch.zeros(y_test[:,::2,::2].shape[0])
# test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(x_test[:,::2,::2], y_test[:,::2,::2]), batch_size=1, shuffle=False)
# with torch.no_grad():
#     for x, y in test_loader:
#         test_l2 = 0
#         x, y = x.to(device), y.to(device)
...
# print('32*25')
# index = 0
# test_e = torch.zeros(y_test[:,::4,::4].shape[0])
# test_loader = torch.utils.data.DataLoader(torch.utils.data.TensorDataset(x_test[:,::4,::4], y_test[:,::4,::4]), batch_size=1, shuffle=False)
# with torch.no_grad():
#     for x, y in test_loader:
#         test_l2 = 0
#         x, y = x.to(device), y.to(device)
