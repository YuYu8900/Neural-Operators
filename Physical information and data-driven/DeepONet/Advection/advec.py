...
class PI_DeepONet:
    ...
    # Define data loss   
    def compute_data_loss(self, params, idx, u0, usol, Nx=100, Nt=100):
    ...
    def loss_data(self, params, idx, u0, usol, m=100, P=100):
    ...
    def train(self, u0, usol, bcs_dataset, res_dataset, nIter = 10000):
        ...
        for it in pbar:
            ...
            if it % 1000 ==0:
                ...
                np.save('adv_eqn_data_params.npy', flat_params)
                np.save('adv_eqn_data_losses.npy', np.array([model.loss_log]))
...
def generate_one_bcs_training_data(key, u0, m=100, P=200):
...
# Load data
path = 'time_advection/Advection_1200_100_100_0.2.mat'#'Burger.mat'  # Please use the matlab script to generate data
data = scipy.io.loadmat(path)

usol = np.array( data['UU'])
x = np.array( data['x'])
t = np.array( data['t'])
u0 = np.array( data['u'])
...
# model.train(u0_train,usol_train, bcs_dataset, res_dataset, nIter=100000)
...
# flat_params, _  = ravel_pytree(model.get_params(model.opt_state))
# np.save('adv_eqn_data_params.npy', flat_params)
# np.save('adv_eqn_data_losses',np.array([model.loss_log]))


# Restore the trained model
flat_params = np.load('adv_eqn_params.npy')
